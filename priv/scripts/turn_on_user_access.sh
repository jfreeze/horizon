...
# Define the pattern to search for (commented out line)
PATTERN="^[[:space:]]*#[[:space:]]*host[[:space:]]+all[[:space:]]+$DB_USER[[:space:]]+0\.0\.0\.0/0[[:space:]]+md5"

# Check if the line exists (commented out)
